...
- The deployed `uk_tenders_public` must be **rebuilt from the raw event log** (no re-scrape needed) for already-loaded rows to reflect the verbatim policy; until that rebuild runs, previously-redacted OCDS rows remain stripped. The rebuild procedure is documented in [DEPLOYMENT.md](../../DEPLOYMENT.md) ("Rebuild the public dataset from the raw event log").
